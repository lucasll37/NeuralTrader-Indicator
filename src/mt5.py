...
def printInfo():
    ...
    try:
        deals = mt5.history_deals_get(datetime.now(pytz.timezone(
            'Etc/GMT-3')) - timedelta(days=1), datetime.now(pytz.timezone('Etc/GMT-3')))
        ...
        deals = mt5.history_deals_get(
            startDate, datetime.now(pytz.timezone('Etc/GMT-3')))
        df = pd.DataFrame(list(deals), columns=deals[0]._asdict().keys())
        dfProfit = df[df['profit'] > 0]['profit']
        dfLoss = df[df['profit'] < 0]['profit']
...
def updateSL():
    ...
    if len(positions) == 1:
        ...
        newSL = 2 * position.price_open - position.tp + trailStopSpeed * \
